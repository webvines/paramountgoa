...
class FusionSC_Person {
    ...
    function render( $args, $content = '') {
		...
		if( $name || $title || $content ) {
			...
			$inner_content .= sprintf( '<div %s><div %s><div %s><span %s>%s</span><span %s>%s</span></div><div %s>%s</div></div><div %s>%s</div></div>', 
									   ...
									   FusionCore_Plugin::attributes( 'person-title' ), $title,
							  		   FusionCore_Plugin::attributes( 'person-shortcode-social-networks' ), $icons,
