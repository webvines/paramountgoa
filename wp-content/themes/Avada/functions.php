...
if( ! $smof_data['status_opengraph'] ) {
	...
	function fusion_insert_og_meta() {
		...
		echo sprintf( '<meta property="og:title" content="%s"/>', $post->post_title );
...
function avada_admin_notice() {
	...
	if( isset($_GET['imported']) && $_GET['imported'] == 'success' ) {
        echo '<div id="setting-error-settings_updated" class="updated settings-error"><p>';
        printf(__('Sucessfully imported demo data! Now, please install and run <a href="' . admin_url( 'plugin-install.php?tab=plugin-information&amp;plugin=regenerate-thumbnails&amp;TB_iframe=true&amp;width=830&amp;height=472' ) . '" class="thickbox" title="Regenerate Thumbnails">Regenerate Thumbnails</a> plugin once.'));
        echo "</p></div>";
	}
}
